...
/// Only parse rules and queries for this segment
pub fn extend_program(
    ...
    let mut parsed =
        Datalog::parse(Rule::program, source).map_err(|e| Error::ParserError(Box::new(e)))?;
    let _matched_str = parsed.as_str();
    let pair = parsed.next().unwrap();

    parse_into_program_only_rules(pair, program)?;
...
fn parse_into_program_only_rules(input_pair: Pair<'_, Rule>, program: &mut Program) -> Result<()> {
    match_then_any! {
        input_pair => program ;
        program ;
        rule => parse_rule ;
